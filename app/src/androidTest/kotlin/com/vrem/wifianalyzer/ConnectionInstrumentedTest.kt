...
internal class ConnectionInstrumentedTest : Runnable {
    override fun run() {
        ...
        verifyIpAddressDisplayed()
        verifyConnectionPopup()
    }

    private fun verifyCurrentConnection() {
        onView(withId(R.id.connection)).check(matches(isDisplayed()))
        onView(withText("Current connection")).check(matches(isDisplayed()))
    }
    ...
    private fun verifyConnectionPopup() {
        onView(withId(R.id.connection)).perform(click())
        pauseShort()
        dismissPopup()
    }
}
