...
@OpenClass
class SeriesCache {
    private val cache: MutableMap<WiFiDetail, BaseSeries<DataPoint>> = mutableMapOf()

    fun difference(series: Set<WiFiDetail>): List<WiFiDetail> = cache.keys.minus(series).toList()

    fun remove(series: List<WiFiDetail>): List<BaseSeries<DataPoint>> =
    ...
    operator fun get(wiFiDetail: WiFiDetail): BaseSeries<DataPoint> = cache[wiFiDetail]!!

    fun put(wiFiDetail: WiFiDetail, series: BaseSeries<DataPoint>) =
